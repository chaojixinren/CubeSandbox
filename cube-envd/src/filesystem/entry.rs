...
//! Disk metadata to the `EntryInfo` message on the wire: file type, mode,
//! size, permission string, owner/group and RFC3339 modification time.
//!
//! Baseline-verified EntryInfo JSON (2026.16 image):
//! ```json
...
//! ```
//! Note `size` is a string (proto3 JSON int64) and `mode` is decimal; owner
//! and group fall back to the numeric id when the name table has no entry.

use crate::filesystem::wire::EntryInfo;
