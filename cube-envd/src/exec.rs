...
pub fn spawn(
    ...
    let (tx, rx) = mpsc::channel::<PumpEvent>(64);
    ...
    tokio::spawn(async move {
        ...
        match child.wait().await {
            Ok(status) => {
                let _ = tx
                    .send(PumpEvent::End(EndEvent::from_exit_status(status)))
                    .await;
            }
            Err(e) => {
                let _ = tx
                    .send(PumpEvent::SpawnError(format!("wait failed: {e}")))
                    .await;
            }
        }
    });

    Ok(SpawnedProcess { pid, events: rx })
}

async fn pump_pipe<R>(pipe: Option<R>, tx: mpsc::Sender<PumpEvent>, is_stderr: bool)
where
    R: tokio::io::AsyncRead + Unpin,
{
    ...
    // Set once the receiver goes away (client disconnected): keep draining so
    // the child never blocks on a full pipe, but skip encoding/sending.
    ...
    loop {
        match pipe.read(&mut buf).await {
            ...
            Ok(n) => {
                ...
                if tx.send(PumpEvent::Data(event)).await.is_err() {
                    receiver_gone = true;
                }
            }
        }
