...
#[cfg(test)]
mod bench {
    //! Ignored micro-benchmarks behind this branch's per-frame numbers. They
    //! need a release build to mean anything:
    //!
    //! ```text
    //! cargo test --release --bin cube-envd -- --ignored --nocapture frame_encode
    //! ```

    use super::*;
    use base64::Engine;

    /// Cost of framing one read chunk, split by the three steps the
    /// single-buffer path removed two full copies from.
    #[test]
    #[ignore]
    fn frame_encode_cost_per_chunk() {
        let chunk = vec![0x41u8; 128 * 1024];
        let reps = 2000;
        let b64 = base64::engine::general_purpose::STANDARD.encode(&chunk);
        let event = || {
            Event::Data(crate::process::wire::DataEvent {
                stdout: Some(b64.clone()),
                ..Default::default()
            })
        };
        let time = |label: &str, f: &dyn Fn()| {
            ...
            println!("{:24} {:?}/frame", label, start.elapsed() / reps);
        };
        time("base64_encode", &|| {
            let _ = base64::engine::general_purpose::STANDARD.encode(&chunk);
        });
        time("event_frame", &|| {
            let _ = event_frame(event());
        });
        let value = serde_json::to_value(EventEnvelope { event: event() }).unwrap();
        time("value_path (to_string)", &|| {
            let _ = protocol::message_frame(&value);
        });
        let frame = event_frame(event());
        println!(
            "{} B read -> {} B frame ({} B payload)",
            chunk.len(),
            frame.len(),
            frame.len() - crate::protocol::frames::FRAME_HEADER_LEN
        );
    }
}
