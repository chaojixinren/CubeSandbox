...
mod tests {
    ...
    async fn full_response_queue_reserves_an_explicit_error_trailer() {
        ...
        let frame = connect::message_frame(&serde_json::json!({"change": "created"}));
    ...
    async fn final_message_and_trailer_fit_one_remaining_slot() {
        for error in [
            ...
            let mut output = Some(sender);
            let first = connect::message_frame(&serde_json::json!({"change": "created"}));
            assert!(try_send_data_frame(&mut output, first.clone()));
            let last = connect::message_frame(&serde_json::json!({"complete": true}));
            let trailer = error
                .as_ref()
                .map(connect::end_stream_error)
                .unwrap_or_else(connect::end_stream_ok);
    ...
    async fn response_helpers_preserve_http_and_wire_contracts() {
        ...
        let body = axum::body::to_bytes(response.into_body(), 1024)
            .await
            .unwrap();
        assert_eq!(body, connect::end_stream_error(&error));
        ...
        assert_eq!(&body[message.len()..], connect::end_stream_ok().as_ref());
    }
}
